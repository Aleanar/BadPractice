	...
	<body>
		...
		<div id="show-tag" class="content scaffold-show" role="main">
			...
			<ol class="property-list tag">
				...
				<li class="fieldcontain">
					<span id="threads-label" class="property-label"><g:message code="tag.threads.label" default="Threads" /></span>
					
						<g:each in="${tagInstance.threads}" var="t">
						<span class="property-value" aria-labelledby="threads-label"><g:link controller="thread" action="show" id="${t.id}">${t?.encodeAsHTML()}</g:link></span>
						</g:each>
