	...
	<body>
		...
		<div id="show-vote" class="content scaffold-show" role="main">
			...
			<ol class="property-list vote">
				...
				<g:if test="${voteInstance?.thread}">
				<li class="fieldcontain">
					<span id="thread-label" class="property-label"><g:message code="vote.thread.label" default="Thread" /></span>
					
						<span class="property-value" aria-labelledby="thread-label"><g:link controller="thread" action="show" id="${voteInstance?.thread?.id}">${voteInstance?.thread?.encodeAsHTML()}</g:link></span>
